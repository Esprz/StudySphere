...
class RedisCache:
    ...
    async def get_or_set(
        ...
        **kwargs
        ...
        return await self.redis.cache_or_fetch(
            key, fetch_func, ttl, *args, **kwargs
        )
    ...
    async def cache_user_recommendations(
        ...
        return await self.redis.set(
            cache_key,
            recommendations,
            ttl or self.redis.CACHE_TTL["recommendations"]
        )
    ...
    async def cache_user_similarities(
        ...
        return await self.redis.set(
            cache_key,
            similarities,
            ttl or self.redis.CACHE_TTL["user_similarities"]
        )
    ...
    async def cache_item_similarities(
        ...
        return await self.redis.set(
            cache_key,
            similarities,
            ttl or self.redis.CACHE_TTL["item_similarities"]
        )
    ...
    async def cache_trending_posts(
        ...
        return await self.redis.set(
            cache_key,
            posts,
            ttl or self.redis.CACHE_TTL["trending_posts"]
        )
    
    async def get_trending_posts(self, category: str = "general") -> Optional[List[Dict[str, Any]]]:
    ...
    async def cache_user_embeddings(
        ...
        return await self.redis.set(
            cache_key,
            embeddings,
            ttl or self.redis.CACHE_TTL["user_embeddings"]
        )
    ...
    async def cache_content_embeddings(
        ...
        return await self.redis.set(
            cache_key,
            embeddings,
            ttl or self.redis.CACHE_TTL["content_embeddings"]
        )
    ...
    async def cache_user_interactions(
        ...
        return await self.redis.set(
            cache_key,
            interactions,
            ttl or self.redis.CACHE_TTL["user_interactions"]
        )
    
    async def get_user_interactions(self, user_id: str) -> Optional[List[Dict[str, Any]]]:
    ...
    async def invalidate_content_data(self, content_id: str) -> int:
        ...
        patterns = [
            f"sim:item:{content_id}",
            f"emb:content:{content_id}"
        ]
        ...
        logger.info(f"Invalidated {total_deleted} cache entries for content {content_id}")
        return total_deleted
